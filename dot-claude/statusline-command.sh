...
if [ -n "$branch" ]; then
  ...
  if [ -n "$stats" ]; then
    ...
    printf "%s (%s) \033[38;2;158;206;106m+%s\033[0m \033[38;2;247;118;142m-%s\033[0m%s%s%s" "$short_dir" "$branch" "$added" "$removed" "$ctx" "$model_str" "$effort_str"
  else
    printf "%s (%s)%s%s%s" "$short_dir" "$branch" "$ctx" "$model_str" "$effort_str"
  fi
else
  printf "%s%s%s%s" "$short_dir" "$ctx" "$model_str" "$effort_str"
fi
